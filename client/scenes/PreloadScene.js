export class PreloadScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // --- Spritesheets ---
    // Tree growth stages: 6 frames wide (empty→harvestable), multiple tree types tall
    ...
    // Weather overlay sprites (rain, sun, cloud)
    ...
    // --- Fonts ---
    this.load.bitmapFont('farm-font', 'assets/fonts/farm-font.png', 'assets/fonts/farm-font.xml')
  }

  create() {
    // Build global sprite animations once, reused across scenes
    this._createAnims()
    this.scene.start('AuthScene')
  }

  _createAnims() {
    // Farmer walk cycles (down / up / left / right)
    const dirs = ['down','up','left','right']
    dirs.forEach((dir, i) => {
      this.anims.create({
        key: `farmer-walk-${dir}`,
        frames: this.anims.generateFrameNumbers('farmer', { start: i * 4, end: i * 4 + 3 }),
    ...
    this.anims.create({
      key: 'water-splash',
      frames: this.anims.generateFrameNumbers('water-splash', { start: 0, end: 7 }),
